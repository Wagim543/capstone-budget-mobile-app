...
class _PersonalScreenState extends State<PersonalScreen> {
  ...
  double get _dailyTarget => _isCampusDay ? _campusDailyRate : _onlineDailyRate;

  // Filter out Vault Sweeps so transferring cash to savings doesn't count as spending
  double get _todaySpent => _todayExpenses
      .where((item) => item.category != 'Vault Sweep')
      .fold(0.0, (sum, item) => sum + item.amount);

  double get _todayRemaining => _dailyTarget - _todaySpent;

  bool get _hasSweptToday =>
      _todayExpenses.any((item) => item.category == 'Vault Sweep');
  ...
  Future<void> _sweepToVault() async {
    if (_hasSweptToday) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text(
            'You have already swept your remaining allowance today!',
          ),
        ),
      );
      return;
                    ...
                    Row(
                      ...
                      children: [
                        ...
                        FilledButton.tonalIcon(
                          onPressed: (_todayRemaining > 0 && !_hasSweptToday)
                              ? _sweepToVault
                              : null,
                          icon: Icon(
                            _hasSweptToday
                          ...
                          label: Text(
                            _hasSweptToday ? 'Swept' : 'Sweep Leftover',
                          ),
                        ),
                      ],
                    ),
                      ...
                      children: [
                        Text(
                          ...
                          style: TextStyle(
                            fontWeight: FontWeight.bold,
                            fontSize: 15,
                            color: isSweep ? Colors.teal : null,
                          ),
                        ),
                        IconButton(
                          icon: const Icon(
